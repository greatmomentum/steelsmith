...
const Menu = ({ isCollapsed }) => {
  return (
    <div className={` bg-gray-800 text-white text-base font-semibold flex flex-col transition-all duration-300   ${isCollapsed ? 'w-0' : 'w-1/6'}`}>
        ...
        {/* row 3 */}
        <div className={`flex items-center justify-center font-light text-white border-t-[1px] border-[#373b52] h-16 ${isCollapsed ? 'hidden' : 'block'}`}>
          <div className='text-xs'>Powered by Momentum ads</div>
        </div>
      </div>
  );
}
